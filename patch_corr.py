...
old_E = """        // Compute E_l (Original Progress Ratio)
...
new_E = """        // Compute E_l as Wasted Effort (1.0 - eta_l)
        std::vector<float> E(L + 1, 0.0f);
        for (int l = L; l >= 1; l--) {
...
if old_E in code:
    ...
    with open('/home/ryawszn/dev/cpp/hnsw-2metric-ef/research/corr.cpp', 'w') as f:
        f.write(code)
    print("Patched corr.cpp to use Improvement Efficiency.")
else:
    print("Could not find old_E block.")
