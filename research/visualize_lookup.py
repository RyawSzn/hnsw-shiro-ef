...
def main():
    ...
    if not os.path.exists(csv_path):
        print(f"Error: {csv_path} not found. Please run the lookup C++ binary first.")
        return
    ...
    axes[0].set_xlabel("Micro Difficulty (m) Quantile -> Harder", fontsize=12)
    axes[0].set_ylabel("Macro Difficulty (M) Quantile -> Harder", fontsize=12)
    ...
    # We use a threshold cmap to highlight where recall is exactly >= 0.95
    ...
    axes[1].set_xlabel("Micro Difficulty (m) Quantile -> Harder", fontsize=12)
    ...
    plt.tight_layout()
    out_path = "research/figures/lookup_ef_heatmap.png"
