...
DEFAULT_OUT_DIR = os.path.join(SCRIPT_DIR, "img", "story")
...
def draw_box(
    ...
    ax.plot(
        [draw_x, draw_x], [w_lo, q1], color=color, lw=1.5, alpha=alpha, zorder=zorder
    )
    ax.plot(
        [draw_x, draw_x], [q3, w_hi], color=color, lw=1.5, alpha=alpha, zorder=zorder
    )
...
def plot_dataset(dataset: str, csv_dir: str, ada_dir: str, out_dir: str):
    ...
    if base_df is not None and not base_df.empty:
        ...
        ax.plot(
            base_x, base_y, color=BASELINE_COLOR, lw=4.5, ls="-", zorder=10, alpha=0.9
        )
    ...
    if ada_df is not None:
        ...
        legend_handles.append(
            mpatches.Patch(
                facecolor=ADA_COLOR, alpha=0.9, label=f"Ada-EF (Time: {x_ada:.2f}s)"
            )
        )
    ...
    if shiro_df is not None:
        ...
        ax.axhline(
            stats["mean"], color=MEAN_REF_COLOR, lw=3.0, ls="--", alpha=0.9, zorder=1
        )
...
def main():
    ...
    datasets = (
        [args.dataset]
        if args.dataset
        else discover_datasets(args.csv_dir, args.ada_dir)
    )
    if not datasets:
        print(
            f"No per_query_results_*.csv files found in {args.csv_dir} or {args.ada_dir}"
        )
        sys.exit(1)
