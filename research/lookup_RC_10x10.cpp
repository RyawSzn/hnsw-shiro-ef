...
int main(int argc, char** argv) {
    ...
    std::string dataset = "deep-image-96-angular"; // Changed default

    if (argc > 1) target_recall = std::stof(argv[1]);
    if (argc > 2) dataset = argv[2];
    
    std::cout << "=== Generating 10x10 Adaptive EF Lookup Table (M_RC x m_LID) ===" << std::endl;
    ...
    std::cout << "Phase 2: Calculating 10x10 Quantile Boundaries..." << std::endl;
    std::vector<float> sorted_M = M_scores;
    std::vector<float> sorted_m = m_scores;
    std::sort(sorted_M.begin(), sorted_M.end());
    std::sort(sorted_m.begin(), sorted_m.end());

    std::vector<float> M_bounds(11);
    std::vector<float> m_bounds(11);
    for(int i=0; i<=10; i++) {
        int idx = std::min((int)(nq - 1), (int)(i * nq / 10.0));
        M_bounds[i] = sorted_M[idx];
        m_bounds[i] = sorted_m[idx];
    }
    M_bounds[10] = std::numeric_limits<float>::max();
    m_bounds[10] = std::numeric_limits<float>::max();

    std::vector<int> buckets[10][10];
    for (int i = 0; i < nq; i++) {
        int bM = 0;
        while(bM < 9 && M_scores[i] > M_bounds[bM+1]) bM++;
        int bm = 0;
        while(bm < 9 && m_scores[i] > m_bounds[bm+1]) bm++;
    ...
    std::vector<std::vector<std::pair<int, float>>> lookup_table(10, std::vector<std::pair<int, float>>(10, {0, 0.0f}));

    std::cout << "=================================================================================================" << std::endl;
    std::cout << "                          10x10 Adaptive EF Lookup Table (ef, actual_recall)                     " << std::endl;
    std::cout << "=================================================================================================" << std::endl;
    
    std::cout << "     |";
    for(int j=0; j<10; j++) std::cout << "    m" << std::setw(2) << std::left << (j+1) << "    |";
    std::cout << "\n-------------------------------------------------------------------------------------------------" << std::endl;

    std::ofstream out("research/lookup_table_RC_10x10.csv");
    ...
    out.close();
    std::cout << "Lookup table saved to research/lookup_table_RC_10x10.csv" << std::endl;
