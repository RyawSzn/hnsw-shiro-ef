...
def main():
    ...
    if not os.path.exists(cv_recall_file):
        print(
            f"Error: {cv_recall_file} does not exist. Please run extract_cv_recall C++ binary first."
        )
        sys.exit(1)
    ...
    if not df_capped.empty:
        if len(df_capped) > 1:
            ...
            print(
                f"Recall vs CV (Capped at 0.25) -> Pearson: {p_cv_capped:.4f} | Spearman: {s_cv_capped:.4f}"
            )
        else:
    ...
    if not df_capped.empty and len(df_capped) > 1:
        ...
        axes[0].set_title(
            f"Recall vs CV (Capped at 0.25)\nPearson: {p_cv_capped:.4f} | Spearman: {s_cv_capped:.4f}",
            fontsize=15,
            fontweight="bold",
            pad=12,
        )
    ...
    axes[0].set_xlabel("CV (Coefficient of Variance)", fontsize=14)
    ...
    axes[1].set_title(
        f"Recall vs LID\nPearson: {p_lid:.4f} | Spearman: {s_lid:.4f}",
        fontsize=15,
        fontweight="bold",
        pad=12,
    )
    ...
    plt.suptitle(
        f"{dataset} Dataset - Comparing Capped CV vs LID",
        fontsize=18,
        fontweight="bold",
        y=1.05,
    )
