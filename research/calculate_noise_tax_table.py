...
for file in sorted(os.listdir(csv_dir)):
    if file.startswith("per_query_baseline_") and file.endswith(".csv"):
        ...
        for ef_star in unique_efs:
            ...
            df_star = df_base_filtered[df_base_filtered["EF"] == ef_star][["QueryID", "Recall", "Latency(ns)"]].copy()
            df_star.rename(columns={"Recall": "Recall_star", "Latency(ns)": "Latency_star"}, inplace=True)
            
            # Merge target recall back to the filtered dataframe
            df_base_filtered = df_base_filtered.merge(df_star[["QueryID", "Recall_star"]], on="QueryID")
            
            # Find minimum EF where Recall >= Recall_star
            df_sufficient = df_base_filtered[df_base_filtered["Recall"] >= df_base_filtered["Recall_star"]]
            ...
            df_ef_i.rename(columns={"EF": "ef_i_star", "Latency(ns)": "Latency_i_star"}, inplace=True)
            ...
            percent_tax = (total_noise_tax_ms / total_latency_star_ms) * 100 if total_latency_star_ms > 0 else 0
