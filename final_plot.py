...
for idx, ds_name in enumerate(datasets_keys):
    ...
    if ds_name == "sift-128-euclidean":
        ...
        bl_efs_shiro = set(shiro.get(ds_name, {}).get("baseline", {}).keys())
        bl_efs_ada = set(ada.get(ds_name, {}).get("baseline", {}).keys())
        bl_efs = sorted(list(bl_efs_shiro.union(bl_efs_ada)))
        ...
        for ef in bl_efs:
            s_times = (
                shiro.get(ds_name, {}).get("baseline", {}).get(ef, {}).get("times", [])
            )
            a_times = (
                ada.get(ds_name, {}).get("baseline", {}).get(ef, {}).get("times", [])
            )

            if not s_times and not a_times:
                continue

            if s_times and a_times:
                s_med_val = s_times[1] if len(s_times) > 1 else s_times[0]
                a_med_val = a_times[1] if len(a_times) > 1 else a_times[0]
                u_med_val = (s_med_val + a_med_val) / 2.0
                total_shiro_med += s_med_val
                total_ada_med += a_med_val
                total_unified_med += u_med_val
            elif s_times:
                s_med_val = s_times[1] if len(s_times) > 1 else s_times[0]
                u_med_val = s_med_val
                total_shiro_med += s_med_val
            ...
            bl_times_med.append(u_med_val / 1000.0)

            target_log = shiro if s_times else ada
            bl_avg.append(target_log[ds_name]["baseline"][ef]["avg"])
            bl_p05.append(target_log[ds_name]["baseline"][ef]["p05"])
            bl_p01.append(target_log[ds_name]["baseline"][ef]["p01"])

        scale_shiro = total_unified_med / total_shiro_med if total_shiro_med else 1.0
        scale_ada = total_unified_med / total_ada_med if total_ada_med else 1.0
